 ...
 */
package com.livingwater.comment.ui.sectionbar;
...
public class SectionProgressBar extends View {
    ...
    private static final int DEFAULT_TEXT_COLOR = Color.parseColor("#44000000");
    ...
    public SectionProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBarHeight = ta.getDimension(R.styleable.SectionProgressBar_section_bar_height, dp2px(10));
    ...
    private void drawForeground(Canvas canvas) {
        ...
        cursorRect.bottom = (int) (rectF.bottom + dp2px(3f));
    ...
    private void drawSectionAndSubscript(Canvas canvas) {
        ...
        float section_span = (effective_width - effective_width * 0.05f) / (mLevelValues.length - 1);
        //初始化分段符位置
        RectF rectF = new RectF();
        rectF.left = getPaddingLeft() + section_span * 0.05f;
        rectF.top = getHeight() / 3.f - mBarHeight / 2.f;
        rectF.right = rectF.left + section_width;
        rectF.bottom = getHeight() / 3.f + mBarHeight / 2.f;
        //初始化Level位置
        float mLevelX = rectF.centerX();
        float mLevelY = rectF.bottom + dp2px(15f);
